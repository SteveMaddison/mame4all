...
static void updatekeyboard(void)
{
	...
	//key[KEY_O]=ExKey1 & PND_LEFT;
	//key[KEY_K]=ExKey1 & PND_RIGHT;

	//key[KEY_1]=((ExKey1 & PND_START) && (!(ExKey1 & PND_SELECT)));
	//key[KEY_2]=((ExKey2 & PND_START) && (!(ExKey2 & PND_SELECT)));
	//key[KEY_3]=((ExKey3 & PND_START) && (!(ExKey3 & PND_SELECT)));
	//key[KEY_4]=((ExKey4 & PND_START) && (!(ExKey4 & PND_SELECT)));
	//key[KEY_5]=((!(ExKey1 & PND_START)) && (ExKey1 & PND_SELECT));
	//key[KEY_6]=((!(ExKey2 & PND_START)) && (ExKey2 & PND_SELECT));
	//key[KEY_7]=((!(ExKey3 & PND_START)) && (ExKey3 & PND_SELECT));
	//key[KEY_8]=((!(ExKey4 & PND_START)) && (ExKey4 & PND_SELECT));

	/* Start A == Start Button */
	key[KEY_1]=( (ExKey1 & PND_START) && !(ExKey1 & PND_SELECT) && !(ExKey1 & PND_UP) && !(ExKey1 & PND_RIGHT) && !(ExKey1 & PND_DOWN) );
	/* Start B == Joystick UP + Start Button */
	key[KEY_2]=( ( (ExKey1 & PND_START) && !(ExKey1 & PND_SELECT) && (ExKey1 & PND_UP) && !(ExKey1 & PND_RIGHT) && !(ExKey1 & PND_DOWN) ) || (ExKey2 & PND_START));
	/* Start C == Joystick RIGHT + Start Button */
	key[KEY_3]=( ( (ExKey1 & PND_START) && !(ExKey1 & PND_SELECT) && !(ExKey1 & PND_UP) && (ExKey1 & PND_RIGHT) && !(ExKey1 & PND_DOWN) ) || (ExKey3 & PND_START));
	/* Start D == Joystick DOWN + Start Button */
	key[KEY_4]=( ( (ExKey1 & PND_START) && !(ExKey1 & PND_SELECT) && !(ExKey1 & PND_UP) && !(ExKey1 & PND_RIGHT) && (ExKey1 & PND_DOWN) ) || (ExKey4 & PND_START));
	/* Coin A == Select Button */
	key[KEY_5]=( !(ExKey1 & PND_START) && (ExKey1 & PND_SELECT) && !(ExKey1 & PND_UP) && !(ExKey1 & PND_RIGHT) && !(ExKey1 & PND_DOWN) );
	/* Coin B == Select Button + Joystick UP */	
	key[KEY_6]=( ( !(ExKey1 & PND_START) && (ExKey1 & PND_SELECT) && (ExKey1 & PND_UP) && !(ExKey1 & PND_RIGHT) && !(ExKey1 & PND_DOWN) ) || (ExKey2 & PND_SELECT));
	/* Coin C == Select Button + Joystick RIGHT */
	key[KEY_7]=( ( !(ExKey1 & PND_START) && (ExKey1 & PND_SELECT) && !(ExKey1 & PND_UP) && (ExKey1 & PND_RIGHT) && !(ExKey1 & PND_DOWN) ) || (ExKey3 & PND_SELECT));
	/* Coin D == Select Button + Joystick DOWN */
	key[KEY_8]=( ( !(ExKey1 & PND_START) && (ExKey1 & PND_SELECT) && !(ExKey1 & PND_UP) && !(ExKey1 & PND_RIGHT) && (ExKey1 & PND_DOWN) ) || (ExKey4 & PND_SELECT));

	key[KEY_TAB]=(ExKey1 & PND_START) && (ExKey1 & PND_SELECT);
	key[KEY_ENTER]=(ExKey1 & PND_B);
	key[KEY_ESC]=((ExKey1 & PND_L) && (ExKey1 & PND_R) && (ExKey1 & PND_START));
	key[KEY_P]=((ExKey1 & PND_L) && (ExKey1 & PND_R) && (!(ExKey1 & PND_START)));

    key[KEY_F11]=(((ExKey1 & PND_L) && (ExKey1 & PND_START)) || ((ExKey1 & PND_R) && (ExKey1 & PND_SELECT)));
    key[KEY_LSHIFT]=((ExKey1 & PND_L) && (ExKey1 & PND_START));
}
...
static int is_joy_axis_pressed (int axis, int dir, int ExKey)
{
	...
	else
	{
		...
		else
		{
			switch (dir)
			{
				case 1: return ExKey & PND_RIGHT; break;
				case 2: return ExKey & PND_LEFT; break;
				default: return 0; break;
			}
		}
	}
